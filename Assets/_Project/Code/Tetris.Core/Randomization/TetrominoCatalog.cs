    ...
    public sealed class TetrominoCatalog : ITetrominoCatalog
    {
        ...
        public TetrominoCatalog(TetrominoSetSO set)
        {
            ...
            for (var i = 0; i < shapes.Length; i++)
            {
                var shape = shapes[i];
                if (shape == null)
                    throw new ArgumentException($"TetrominoSet contains a null shape at index {i}", nameof(set));

                if (_shapeByType.ContainsKey(shape.Type))
                    throw new ArgumentException($"TetrominoSet contains duplicate shapes for type '{shape.Type}'.", nameof(set));
